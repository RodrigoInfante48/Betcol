        ...
        <div className="grid grid-cols-2 gap-x-2 gap-y-0.5">
          {teams.map((t, i) => (
            <button
              key={t}
              onClick={() => onTeamClick(t)}
              className="flex items-center gap-1.5 py-0.5 hover:text-green-400 transition-colors text-left w-full group/team"
            >
              <span className="text-gray-600 text-[10px] w-3">{i + 1}.</span>
              <span className="text-[10px] text-gray-400">{TEAMS[t]?.flag}</span>
              <span className="text-xs text-gray-300 group-hover/team:text-green-400 transition-colors">{t}</span>
            </button>
          ))}
        </div>
      </div>
