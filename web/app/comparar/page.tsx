  ...
  return (
    <div className="mb-4">
      <div className="flex items-center justify-between text-sm mb-2">
        <span className={`font-bold ${homeWins ? 'text-blue-500 dark:text-blue-400' : 'text-gray-500 dark:text-gray-400'}`}>{f(homeVal)}</span>
        <span className="text-gray-400 dark:text-gray-500 text-xs uppercase tracking-wide">{label}</span>
        <span className={`font-bold ${awayWins ? 'text-orange-400' : 'text-gray-500 dark:text-gray-400'}`}>{f(awayVal)}</span>
      </div>
      <div className="flex h-2.5 rounded-full overflow-hidden bg-gray-200 dark:bg-gray-800">
        <div
...
function TeamSelector({ value, onChange, label, color }: {
  ...
  const textCls = color === 'blue' ? 'text-blue-500 dark:text-blue-400' : 'text-orange-400'
    ...
    <div className="flex-1">
      ...
      <div className="relative">
        ...
        <select
          ...
          className={`w-full pl-10 pr-4 py-3 bg-gray-200 dark:bg-gray-800 border ${borderCls} rounded-xl text-sm font-medium ${textCls} focus:outline-none transition-colors appearance-none`}
        >
          {ALL_TEAM_NAMES.map(name => (
            <option key={name} value={name} className="text-gray-800 dark:text-gray-200 bg-gray-200 dark:bg-gray-800">
...
export default function CompararPage() {
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100 mb-1">Comparar Equipos</h1>
        <p className="text-gray-500 dark:text-gray-400 text-sm">Selecciona dos selecciones para comparar su historial y ver las probabilidades del enfrentamiento</p>
      </div>

      {/* Team selectors */}
      <div className="bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-5 mb-6">
        <div className="flex items-center gap-4">
          ...
          <div className="flex flex-col items-center gap-2 mt-4">
            <button
              ...
              className="p-2 rounded-full border border-gray-300 dark:border-gray-700 text-gray-500 dark:text-gray-400 hover:border-gray-400 dark:hover:border-gray-500 hover:text-gray-800 dark:hover:text-gray-200 transition-colors"
            ...
            <span className="text-xs text-gray-300 dark:text-gray-700">vs</span>
          </div>
          <TeamSelector value={awayTeam} onChange={v => { if (v !== homeTeam) setAwayTeam(v) }} label="Equipo B" color="orange" />
        </div>

        {homeTeam === awayTeam && (
          <p className="text-yellow-600 dark:text-yellow-400 text-xs mt-3 text-center">Selecciona equipos diferentes</p>
        )}
      </div>
      ...
      {home && away && probs && homeTeam !== awayTeam && (
        <>
          ...
          {sameGroup && scheduledMatch && (
            <div className="mb-5 p-4 bg-green-500/15 border border-green-500/25 rounded-xl">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs text-green-700 dark:text-green-400 font-semibold uppercase tracking-wide mb-1">
                ...
                <Link
                  ...
                  className="text-xs px-3 py-2 bg-green-500/20 border border-green-500/40 text-green-700 dark:text-green-400 rounded-lg hover:bg-green-500/30 transition-colors"
                >
                  Ver análisis →
                </Link>
              </div>
          ...
          {!sameGroup && (
            <div className="mb-5 p-3 bg-gray-50/60 dark:bg-gray-900/60 border border-gray-200 dark:border-gray-800 rounded-xl">
          ...
          <div className="grid grid-cols-3 gap-3 mb-6">
            {[
              { label: `Gana ${home.name}`, p: pctNum(probs.pHome), flag: home.flag, color: 'text-blue-500 dark:text-blue-400', border: 'border-blue-500/20', bg: 'bg-blue-500/10 dark:bg-blue-900/10' },
              { label: 'Empate', p: pctNum(probs.pDraw), flag: '🤝', color: 'text-gray-600 dark:text-gray-300', border: 'border-gray-300 dark:border-gray-700', bg: 'bg-gray-100/20 dark:bg-gray-800/20' },
              { label: `Gana ${away.name}`, p: pctNum(probs.pAway), flag: away.flag, color: 'text-orange-400', border: 'border-orange-500/20', bg: 'bg-orange-500/10 dark:bg-orange-900/10' },
            ].map(opt => {
              ...
              return (
                <div key={opt.label} className={`${opt.bg} border ${isMax ? 'border-green-500/40 ring-1 ring-green-500/20' : opt.border} rounded-xl p-4 text-center`}>
                  ...
                  <p className="text-xs text-gray-400 dark:text-gray-500 mt-1 leading-tight">{opt.label}</p>
                  {isMax && <p className="text-[10px] text-green-700 dark:text-green-400 mt-1 font-semibold">Favorito</p>}
                </div>
              )
            })}
          </div>
          ...
          <div className="bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-5 mb-6">
            <h3 className="text-sm font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-5">
              Comparativa estadística <span className="text-gray-400 dark:text-gray-600 font-normal">(últimos 10 partidos)</span>
            </h3>
              ...
              return (
                <div key={team.name} className={`bg-gray-50 dark:bg-gray-900 border ${borderCls} rounded-2xl p-5`}>
                  ...
                  <div className="grid grid-cols-3 gap-3 text-center mb-4">
                    <div className="bg-gray-200/50 dark:bg-gray-800/50 rounded-xl py-3">
                      <p className="text-2xl font-extrabold text-green-600 dark:text-green-400">{team.wins}</p>
                      <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">Victorias</p>
                    </div>
                    <div className="bg-gray-200/50 dark:bg-gray-800/50 rounded-xl py-3">
                      <p className="text-2xl font-extrabold text-yellow-600 dark:text-yellow-400">{team.draws}</p>
                      <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">Empates</p>
                    </div>
                    <div className="bg-gray-200/50 dark:bg-gray-800/50 rounded-xl py-3">
                      <p className="text-2xl font-extrabold text-red-600 dark:text-red-400">{team.losses}</p>
                      <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">Derrotas</p>
                    </div>
                  </div>

                  <div className="grid grid-cols-2 gap-3 text-center mb-4">
                    <div className="bg-gray-200/30 dark:bg-gray-800/30 rounded-lg py-2.5">
                      <p className="text-lg font-bold text-gray-900 dark:text-gray-100">{(team.goalsFor / 10).toFixed(1)}</p>
                      <p className="text-[10px] text-gray-400 dark:text-gray-500">Goles/partido</p>
                    </div>
                    <div className="bg-gray-200/30 dark:bg-gray-800/30 rounded-lg py-2.5">
                      <p className="text-lg font-bold text-gray-900 dark:text-gray-100">{(team.goalsAgainst / 10).toFixed(1)}</p>
                      <p className="text-[10px] text-gray-400 dark:text-gray-500">Recibidos/partido</p>
                    </div>
                  </div>

                  <div>
                    <p className="text-xs text-gray-400 dark:text-gray-500 mb-2">Forma reciente</p>
          ...
          <div className="bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-5">
            <h3 className="text-sm font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-4">Resumen de apuestas</h3>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-center">
              <div className="bg-gray-200/40 dark:bg-gray-800/40 rounded-xl p-3">
                <p className="text-[10px] text-gray-400 dark:text-gray-500 mb-1">Más de 2.5 goles</p>
                <p className="text-xl font-extrabold text-green-600 dark:text-green-400">{pctNum(probs.pOver25)}%</p>
              </div>
              <div className="bg-gray-200/40 dark:bg-gray-800/40 rounded-xl p-3">
                <p className="text-[10px] text-gray-400 dark:text-gray-500 mb-1">Menos de 2.5 goles</p>
                <p className="text-xl font-extrabold text-red-600 dark:text-red-400">{pctNum(probs.pUnder25)}%</p>
              </div>
              <div className="bg-gray-200/40 dark:bg-gray-800/40 rounded-xl p-3">
                <p className="text-[10px] text-gray-400 dark:text-gray-500 mb-1">Ambos anotan</p>
                <p className="text-xl font-extrabold text-yellow-600 dark:text-yellow-400">{pctNum(probs.pBothScore)}%</p>
              </div>
              <div className="bg-gray-200/40 dark:bg-gray-800/40 rounded-xl p-3">
                <p className="text-[10px] text-gray-400 dark:text-gray-500 mb-1">Goles esperados</p>
                <p className="text-xl font-extrabold text-gray-900 dark:text-gray-100">{probs.expectedTotal.toFixed(1)}</p>
              </div>
            </div>
          </div>
        </>
      ...
      <p className="text-xs text-gray-300 dark:text-gray-700 mt-6">
