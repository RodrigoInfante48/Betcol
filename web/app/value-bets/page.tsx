...
function MetricCard({ label, value, sub }: { label: string; value: string | number; sub?: string }) {
  return (
    <div className="bg-gray-200 dark:bg-gray-800 rounded-lg p-4 border border-gray-300 dark:border-gray-700">
      <p className="text-gray-500 dark:text-gray-400 text-xs uppercase tracking-wide">{label}</p>
      <p className="text-2xl font-bold text-green-600 dark:text-green-400 mt-1">{value}</p>
...
export default function ValueBetsPage() {
  ...
  if (loading) return <div className="text-gray-500 dark:text-gray-400 mt-16 text-center">Cargando value bets...</div>
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        <div>
          <label className="block text-xs text-gray-500 dark:text-gray-400 mb-1 uppercase tracking-wide">
            Edge mínimo: <span className="text-green-600 dark:text-green-400">{minEdge}%</span>
          </label>
        ...
        <div>
          <label className="block text-xs text-gray-500 dark:text-gray-400 mb-1 uppercase tracking-wide">
            EV mínimo: <span className="text-green-600 dark:text-green-400">{minEv}%</span>
          </label>
          <input type="range" min={0} max={30} value={minEv}
            onChange={e => setMinEv(Number(e.target.value))} className="w-full accent-green-500" />
        </div>
        <div>
          <label className="block text-xs text-gray-500 dark:text-gray-400 mb-1 uppercase tracking-wide">Mercado</label>
          <select value={market} onChange={e => setMarket(e.target.value)}
            className="w-full bg-gray-200 dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-green-500">
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-16 text-gray-400 dark:text-gray-500 bg-gray-200/30 dark:bg-gray-800/30 rounded-lg border border-gray-200 dark:border-gray-800">
        ...
        <div className="overflow-x-auto rounded-lg border border-gray-200 dark:border-gray-800">
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-200 dark:bg-gray-800 text-gray-500 dark:text-gray-400 text-xs uppercase tracking-wide">
            ...
            <tbody>
              {filtered.map((b, i) => {
                const dateStr = b.event_date
                  ...
                  : '—'
                return (
                  <tr key={b.id} className={`border-t border-gray-200 dark:border-gray-800 hover:bg-gray-200/40 dark:hover:bg-gray-800/40 transition-colors ${i % 2 === 0 ? 'bg-gray-50 dark:bg-gray-900' : 'bg-gray-50/60 dark:bg-gray-900/60'}`}>
                    <td className="px-4 py-3 font-medium">{b.match_label}</td>
                    <td className="px-4 py-3 text-gray-500 dark:text-gray-400">{dateStr}</td>
                    <td className="px-4 py-3">
                      <span className="px-2 py-0.5 bg-blue-500/10 dark:bg-blue-900/50 text-blue-600 dark:text-blue-300 rounded text-xs font-medium">
                    ...
                    <td className="px-4 py-3 text-right tabular-nums text-gray-500 dark:text-gray-400">{b.implied_prob_best != null ? (b.implied_prob_best * 100).toFixed(1) + '%' : '—'}</td>
                    <td className={`px-4 py-3 text-right font-semibold tabular-nums ${b.edge_best > 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
                      {b.edge_best > 0 ? '+' : ''}{(b.edge_best * 100).toFixed(1)}%
                    </td>
                    <td className={`px-4 py-3 text-right font-semibold tabular-nums ${b.best_ev > 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
                      {b.best_ev > 0 ? '+' : ''}{(b.best_ev * 100).toFixed(1)}%
                    </td>
                    <td className="px-4 py-3 text-right tabular-nums text-yellow-600 dark:text-yellow-400">{(b.best_kelly * 100).toFixed(1)}%</td>
                  </tr>
                )
              })}
            </tbody>
      ...
      <div className="mt-6 p-4 bg-gray-200/40 dark:bg-gray-800/40 rounded-lg border border-gray-300 dark:border-gray-700 text-sm text-gray-500 dark:text-gray-400">
        <strong className="text-gray-800 dark:text-gray-200">¿Qué es un value bet?</strong>{' '}
