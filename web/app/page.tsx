  ...
  return (
    <div>
      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-white">Copa Mundial FIFA 2026</h1>
          <p className="text-gray-400 text-sm mt-1">Fase de grupos · Todos los horarios en hora Colombia (GMT-5)</p>
        </div>
        <div className="flex items-center gap-3">
          <button
            onClick={() => fileInputRef.current?.click()}
            className="text-xs px-3 py-2 rounded-lg border border-gray-700 text-gray-400 hover:border-green-500 hover:text-green-400 transition-colors flex items-center gap-2"
          >
      ...
      {uploadMsg && (
        <div className="mb-4 p-3 bg-green-900/30 border border-green-700/40 rounded-lg text-green-300 text-sm">
          {uploadMsg}
        </div>
      )}
      ...
      {Array.from(byDate.entries()).map(([date, matches]) => (
        <div key={date} className="mb-8">
          <h2 className="text-sm font-semibold text-gray-400 uppercase tracking-wider mb-3 pb-2 border-b border-gray-800 capitalize">
            {formatDate(date)}
          </h2>
            ...
            {matches.map(match => {
              ...
              return (
                <Link
                  ...
                  className="group block bg-gray-900 border border-gray-800 rounded-xl p-4 hover:border-green-500/40 hover:bg-gray-900/80 transition-all"
                >
                  {/* Top row */}
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 flex items-center gap-2">
                      ...
                      <div>
                        <p className="text-sm font-semibold text-white leading-tight">{home.name}</p>
                    ...
                    <div className="flex flex-col items-center gap-1.5 min-w-[90px]">
                      <div className="flex gap-1 text-xs font-bold">
                        <span className="text-blue-400 w-8 text-center">{pct(probs.pHome)}</span>
                        <span className="text-gray-500 w-8 text-center">{pct(probs.pDraw)}</span>
                      ...
                      <div className="flex h-1.5 w-full rounded-full overflow-hidden gap-px">
                        ...
                        <div className="bg-gray-600 transition-all" style={{ width: pct(probs.pDraw) }} />
                    ...
                    <div className="flex-1 flex items-center gap-2 flex-row-reverse text-right">
                      ...
                      <div>
                        <p className="text-sm font-semibold text-white leading-tight">{away.name}</p>
                  ...
                  <div className="flex items-center justify-between mt-3 pt-2 border-t border-gray-800/60">
                    <span className="text-[11px] text-gray-500">
                      Favorito: <span className="text-green-400 font-medium">{probs.favoritoName}</span>
                      <span className="text-gray-600 ml-1">({pct(probs.favoritoPct)})</span>
                    </span>
                    <span className="text-[11px] text-gray-600 group-hover:text-green-400 transition-colors flex items-center gap-1">
                      Ver análisis
      ...
      {byDate.size === 0 && (
        <div className="text-center py-16 text-gray-500 bg-gray-900/40 rounded-xl border border-gray-800">
